...
fn write_newick_without_outer_brackets(tree: &Tree, args: &Args) -> Result<()> {
    ...
    if newick.ends_with(");") {
        ...
        std::fs::write(&args.o, &newick_wo)
            .context("Unable to write Newick tree to output file")?;
    } else {
        std::fs::write(&args.o, &newick).context("Unable to write Newick tree to output file")?;
    };
    Ok(())
}
...
fn main() -> Result<()> {
    ...
    write_newick_without_outer_brackets(&tree, &args)?;
    Ok(())
}
